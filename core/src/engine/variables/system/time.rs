...
pub fn resolve(key: &str) -> Option<String> {
    ...
    let methods = match parse_methods(method_str) {
        Ok(m) => m,
        Err(e) => {
            tracing::warn!("Failed to parse time methods from '{}': {}", key, e);
            return None;
        }
    };
        ...
        match method {
            ...
            super::datetime::Method::Calc(args) => {
                dt = match apply_temporal_calc(dt, args) {
                    Ok(new_dt) => new_dt,
                    Err(e) => {
                        tracing::warn!("Failed to calculate time offset '{}': {}", args, e);
                        return None;
                    }
                };
            }
    ...
    match format_temporal(dt, format_str) {
        ...
        Err(e) => {
            tracing::warn!("Failed to format time '{}': {}", format_str, e);
            None
        }
    }
}
...
mod tests {
    ...
    fn test_time_methods() {
        ...
        let err_no_sign = resolve("time.calc(1h)");
