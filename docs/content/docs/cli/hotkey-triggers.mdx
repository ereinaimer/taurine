...
While Word Triggers are activated by typing a trigger character followed by a word (e.g., `>gs `), **Hotkey Triggers** are global keyboard shortcuts that execute a trigger immediately when pressed, regardless of your current text state.

<Callout title="Global Shortcuts" type="info">
  Hotkey triggers are global while the Taurine service is running. They do not require a trigger character and do not use the "Backspace Undo" mechanism because no trigger text was typed.
</Callout>
