...
### Fixed
- **Whitespace No Longer Implies Multiplication**: In the inline calculator, numbers separated only by whitespace (e.g. `2 2`) are no longer multiplied together; such input is left as typed text. Implicit multiplication (e.g. `2(2)`, `2pi`) still works when operands are directly adjacent.
